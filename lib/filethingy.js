 ...
 */
FileThingy.prototype.addSlash_ = function(string) {
    ...
    (function next() {
      ...
      fs.stat(file, function(err, stat) {
        ...
        if (stat && stat.isDirectory()) {
          ...
          self.crawl(self.addSlash_(file), function(err, res) {
            if (err) self.complain(err);
            next();
          });
 ...
 * @private
 */
FileThingy.prototype.chopSlash_ = function(string) {
...
FileThingy.prototype.getDirName_ = function(oldName, done) {
  ...
  var parts = this.chopSlash_(rawName).split('/');
 ...
 * files.
 *
 * NOTE: This MUST be asynchronous, from here on, or it can be very slow.
...
FileThingy.prototype.process = function() {
  ...
  this.processDirectories(function(err) {
    console.log('Done processing directories.')
  ...
  (function next() {
    ...
    fs.mkdir(path, function(err) {
      if (err && self.SAFE_ERRORS.indexOf(err.code) === -1) {
        ...
        console.log('Done adding directory:\t', path);
      }
      next();
    });
  })();
...
FileThingy.prototype.processFiles = function(done) {
  ...
  dirs.forEach(function(dir) {
    ...
    (function next() {
      var file = files[i++];
      if (!file) {
        return done('Successfully queued files for ' + dir);
      }
      ...
      self.queue.addFile(function(cb) {
        ...
        fs.open(fpath, 'w', function(err, fd) {
          if (err && self.SAFE_ERRORS.indexOf(err.code) === -1) {
            // TODO: Handle "ENOENT" error - Directory does not exist.
            /**
             * TODO: EMFILE should be Const
             * THis needs work - first if there was an error, this file needs
             * to be added back to the queue, and then we need to make sure it
             * is indeed closed (check to see if a file descriptor exists).
             */
            if (err.code === 'EMFILE') {
              console.log('Too many simutaneously opened files.')
              setTimeout(function() {
                console.log(fpath + ' added!')
                cb();
              }, 2000);
            }
          }
